...
int CameraDetectionBevComponent::ConvertObjectToPb(
  ...
  const uint64_t e0 = apollo::perf::NowNs();
  if (!object_ptr || !pb_msg) {
    return cyber::FAIL;
  }
  const uint64_t e1 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("perception.camera_detection_bev.extract_objects",
                            0, e1);
...
bool CameraDetectionBevComponent::OnReceiveImage(
  ...
  apollo::perf::AppendPoint("perception.camera_detection_bev.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.camera_detection_bev.receive_images",
                              trace_id, r0, r1);
  ...
  apollo::perf::AppendLatency("perception.camera_detection_bev.detect",
                              trace_id, d0, d1);
  ...
  apollo::perf::AppendLatency("perception.camera_detection_bev.get_tf",
                              trace_id, tf0, tf1);
  ...
  apollo::perf::AppendLatency("perception.camera_detection_bev.transform_to_world",
                              trace_id, w0, w1);
  ...
  apollo::perf::AppendLatency("perception.camera_detection_bev.make_msg",
                              trace_id, m0, m1);
  seq_num_++;
  // Send msg
  ...
  apollo::perf::AppendLatency("perception.camera_detection_bev.write",
                              trace_id, s0, s1);
  apollo::perf::AppendLatency("perception.camera_detection_bev.total",
                              trace_id, t0, s1);
