...
// }
bool MultiSensorFusionComponent::Proc(
    const std::shared_ptr<onboard::SensorFrameMessage>& message) {
  PERF_FUNCTION();
  ...
  apollo::perf::AppendPoint("perception.multi_sensor_fusion.tick",
                            trace_id, t0);
  ...
  std::shared_ptr<PerceptionObstacles> out_message(
      new (std::nothrow) PerceptionObstacles);
  std::shared_ptr<onboard::SensorFrameMessage> viz_message(
      new (std::nothrow) onboard::SensorFrameMessage);

  const uint64_t t1 = apollo::perf::NowNs();
  bool status = InternalProc(message, out_message, viz_message);
  const uint64_t t2 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.multi_sensor_fusion.proc",
                              trace_id, t1, t2);
  ...
  if (status) {
    ...
    if (is_main_sensor) {
      ...
      apollo::perf::AppendLatency("perception.multi_sensor_fusion.write",
                                  trace_id, w0, w1);
      apollo::perf::AppendLatency("perception.multi_sensor_fusion.total",
                                  trace_id, t0, w1);

      if (onboard::FLAGS_obs_enable_visualization) {
        inner_writer_->Write(viz_message);
      }
      if (out_message->has_header()) {
        AINFO << "[TRACE_DEBUG_MSF_OUT] "
              << "trace_id=" << apollo::perf::TraceIdFromHeader(out_message->header())
              << " input_trace_id=" << trace_id
              << " input_sensor_id=" << message->sensor_id_
              << " input_timestamp=" << message->timestamp_
              << " input_lidar_timestamp=" << message->lidar_timestamp_
              << " header=" << out_message->header().DebugString()
              << " obstacle_count=" << out_message->perception_obstacle_size();
      } else {
        AWARN << "[TRACE_DEBUG_MSF_OUT] "
              << "trace_id=" << trace_id
              << " input_trace_id=" << trace_id
              << " input_sensor_id=" << message->sensor_id_
              << " input_timestamp=" << message->timestamp_
...
// }
bool MultiSensorFusionComponent::InternalProc(
    const std::shared_ptr<const SensorFrameMessage>& in_message,
    std::shared_ptr<PerceptionObstacles> out_message,
    std::shared_ptr<SensorFrameMessage> viz_message) {
  {
    std::unique_lock<std::mutex> lock(s_mutex_);
  ...
  apollo::perf::AppendLatency("perception.multi_sensor_fusion.fusion_process",
                              trace_id, f0, f1);

  bool is_main_sensor =
      algorithm::SensorManager::Instance()->IsMainSensor(in_message->sensor_id_);
  if (!is_main_sensor) {
    return true;
  }
  ...
  apollo::perf::AppendLatency("perception.multi_sensor_fusion.roi_check",
                              trace_id, r0, r1);
  ...
  apollo::perf::AppendLatency("perception.multi_sensor_fusion.serialize",
                              trace_id, s0, s1);
