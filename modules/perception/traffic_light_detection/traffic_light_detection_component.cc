...
bool TrafficLightDetectComponent::Proc(
  ...
  apollo::perf::AppendPoint("perception.traffic_light_detection.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_detection.proc",
                              trace_id, t1, t2);
  ...
  if (status) {
    ...
    apollo::perf::AppendLatency("perception.traffic_light_detection.write",
                                trace_id, w0, w1);
    apollo::perf::AppendLatency("perception.traffic_light_detection.total",
                                trace_id, t0, w1);
    AINFO << "Send trafficlight detect output message.";
  }
  return status;
...
bool TrafficLightDetectComponent::InternalProc(
  ...
  apollo::perf::AppendLatency("perception.traffic_light_detection.detector",
                              trace_id, d0, d1);
  return status;
}
