...
void TrafficLightsPerceptionComponent::OnReceiveImage(
  ...
  apollo::perf::AppendPoint("perception.traffic_light_region_proposal.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.check_camera",
                              trace_id, c0, c1);
  ...
  const uint64_t u0 = apollo::perf::NowNs();
  if (!UpdateCameraSelection(image_msg_ts, preprocess_option, frame)) {
    AWARN << "add_cached_camera_selection failed, ts: " << image_msg_ts;
  }
  const uint64_t u1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.update_camera_selection",
                              trace_id, u0, u1);
  ...
  const uint64_t sy0 = apollo::perf::NowNs();
  bool sync_image_ok =
      preprocessor_->SyncInformation(image_msg_ts, camera_name);
  const uint64_t sy1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.sync_info",
                              trace_id, sy0, sy1);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.fill_image",
                              trace_id, f0, f1);
  ...
  const uint64_t v0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("perception.traffic_light_region_proposal.timestamp_monitor",
                            trace_id, v0);
  ...
  const uint64_t v1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.verify_projection",
                              trace_id, v0, v1);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.write",
                              trace_id, w0, w1);
  apollo::perf::AppendLatency("perception.traffic_light_region_proposal.total",
                              trace_id, t0, w1);
