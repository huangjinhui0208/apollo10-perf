...
void MotionServiceComponent::OnReceiveImage(const ImageMsgType &message,
  ...
  uint64_t trace_id = 0;
  if (message != nullptr && message->measurement_time() > 0.0) {
    trace_id = static_cast<uint64_t>(message->measurement_time() * 1e9);
  }

  const uint64_t t0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("perception.motion_service.image_tick",
                            trace_id, t0);
...
void MotionServiceComponent::OnLocalization(
  ...
  const uint64_t t0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("perception.motion_service.localization_tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.motion_service.process_status",
                              trace_id, p0, p1);

  if (start_flag_) {
    const uint64_t a0 = apollo::perf::NowNs();
    ...
    const uint64_t a1 = apollo::perf::NowNs();
    apollo::perf::AppendLatency("perception.motion_service.add_motion",
                                trace_id, a0, a1);
  }
...
void MotionServiceComponent::PublishEvent(const double timestamp) {
  uint64_t trace_id = static_cast<uint64_t>(timestamp * 1e9);

  const uint64_t t0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("perception.motion_service.publish_tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.motion_service.get_buffer",
                              trace_id, b0, b1);
  ...
  apollo::perf::AppendLatency("perception.motion_service.write",
                              trace_id, w0, w1);
  apollo::perf::AppendLatency("perception.motion_service.total",
                              trace_id, t0, w1);
}
