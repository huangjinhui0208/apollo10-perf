...
// }
bool TrafficLightRecognComponent::Proc(
    const std::shared_ptr<apollo::perception::onboard::TrafficDetectMessage>& message) {
  PERF_FUNCTION();
  ...
  apollo::perf::AppendPoint("perception.traffic_light_recognition.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_recognition.proc",
                              trace_id, t1, t2);
  ...
  if (status) {
    ...
    apollo::perf::AppendLatency("perception.traffic_light_recognition.write",
                                trace_id, w0, w1);
    apollo::perf::AppendLatency("perception.traffic_light_recognition.total",
                                trace_id, t0, w1);
    AINFO << "Send trafficlight recognition output message.";
  }
  return status;
...
// }
bool TrafficLightRecognComponent::InternalProc(
    const std::shared_ptr<const apollo::perception::onboard::TrafficDetectMessage>& message) {
  uint64_t trace_id = 0;
  if (message != nullptr && message->timestamp_ > 0.0) {
    trace_id = static_cast<uint64_t>(message->timestamp_ * 1e9);
  }
  ...
  apollo::perf::AppendLatency("perception.traffic_light_recognition.recognize",
                              trace_id, r0, r1);
  return status;
}
