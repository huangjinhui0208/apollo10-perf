...
bool LidarOutputComponent::Proc(
  ...
  apollo::perf::AppendPoint("perception.lidar_output.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.lidar_output.serialize",
                              trace_id, e0, e1);

  const uint64_t w0 = apollo::perf::NowNs();
  writer_->Write(out_message);
  const uint64_t w1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.lidar_output.write",
                              trace_id, w0, w1);
  apollo::perf::AppendLatency("perception.lidar_output.total",
                              trace_id, t0, w1);
