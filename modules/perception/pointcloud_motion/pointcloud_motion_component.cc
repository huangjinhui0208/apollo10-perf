...
bool PointCloudMotionComponent::Proc(const std::shared_ptr<LidarFrameMessage>& message) {
  ...
  apollo::perf::AppendPoint("perception.pointcloud_motion.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.pointcloud_motion.internal_proc",
                              trace_id, i0, i1);
  ...
  if (status) {
    ...
    apollo::perf::AppendLatency("perception.pointcloud_motion.write",
                                trace_id, w0, w1);
    apollo::perf::AppendLatency("perception.pointcloud_motion.total",
                                trace_id, t0, w1);
...
bool PointCloudMotionComponent::InternalProc(const std::shared_ptr<LidarFrameMessage>& in_message,
  ...
  apollo::perf::AppendLatency("perception.pointcloud_motion.parse",
                              trace_id, p0, p1);
