...
bool LidarTrackingComponent::Proc(
  ...
  apollo::perf::AppendPoint("perception.lidar_tracking.tick", trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.lidar_tracking.proc",
                              trace_id, t1, t2);
  ...
  if (status) {
    ...
    apollo::perf::AppendLatency("perception.lidar_tracking.write",
                                trace_id, t3, t4);
    
    return true;
  }
  const uint64_t tt0 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.lidar_tracking.total",
                                trace_id, t0, tt0);
  AERROR << "Send lidar tracking output message failed!";
  return false;
}
...
bool LidarTrackingComponent::InternalProc(
  ...
  apollo::perf::AppendLatency("perception.lidar_tracking.multi_target_tracker",
                              trace_id, tr0, tr1);
