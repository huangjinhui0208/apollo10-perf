...
bool RadarDetectionComponent::Proc(
  ...
  if (message != nullptr && message->has_header()) {
    trace_id = apollo::perf::TraceIdFromHeader(message->header());
    if (trace_id == 0 && message->header().timestamp_sec() > 0.0) {
      trace_id = static_cast<uint64_t>(message->header().timestamp_sec() * 1e9);
    }
  }

  const uint64_t t0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("perception.radar_detection.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.radar_detection.proc",
                              trace_id, t1, t2);
  ...
  apollo::perf::AppendLatency("perception.radar_detection.write",
                              trace_id, w0, w1);
  apollo::perf::AppendLatency("perception.radar_detection.total",
                              trace_id, t0, w1);
  return true;
}
...
// }
bool RadarDetectionComponent::InternalProc(
    const std::shared_ptr<apollo::drivers::ContiRadar>& in_message,
    std::shared_ptr<onboard::SensorFrameMessage> out_message) {
  PERF_FUNCTION_WITH_INDICATOR(radar_info_.name);

  uint64_t trace_id = 0;
  if (in_message != nullptr && in_message->has_header()) {
    trace_id = apollo::perf::TraceIdFromHeader(in_message->header());
    if (trace_id == 0 && in_message->header().timestamp_sec() > 0.0) {
      trace_id = static_cast<uint64_t>(in_message->header().timestamp_sec() * 1e9);
    }
  }
  ...
  apollo::perf::AppendLatency("perception.radar_detection.preprocessor",
                              trace_id, p0, p1);
  ...
  Eigen::Affine3d radar2novatel_trans;
  if (!radar2novatel_trans_.GetTrans(timestamp, &radar2novatel_trans,
                                     "novatel", tf_child_frame_id_)) {
    out_message->error_code_ = apollo::common::ErrorCode::PERCEPTION_ERROR_TF;
    AERROR << "Failed to get radar2novatel trans at time: " << timestamp;
    return true;
  }
  const uint64_t tf1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.radar_detection.get_tf",
                              trace_id, tf0, tf1);
  ...
  apollo::perf::AppendLatency("perception.radar_detection.get_car_speed",
                              trace_id, s0, s1);
  ...
  apollo::perf::AppendLatency("perception.radar_detection.get_roi",
                              trace_id, r0, r1);

  std::vector<base::ObjectPtr> radar_objects;
  const uint64_t pr0 = apollo::perf::NowNs();
  if (!radar_perception_->Perceive(corrected_obstacles, options,
                                   &radar_objects)) {
    out_message->error_code_ =
        apollo::common::ErrorCode::PERCEPTION_ERROR_PROCESS;
  ...
  const uint64_t pr1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.radar_detection.perception",
                              trace_id, pr0, pr1);
