...
bool TrafficLightTrackComponent::Proc(
  ...
  apollo::perf::AppendPoint("perception.traffic_light_tracking.tick",
                            trace_id, t0);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_tracking.proc",
                              trace_id, t1, t2);

  if (status) {
    const uint64_t w0 = apollo::perf::NowNs();
    // writer_->Write(out_msg) is inside InternalProc
    const uint64_t w1 = apollo::perf::NowNs();
    apollo::perf::AppendLatency("perception.traffic_light_tracking.total",
                                trace_id, t0, w1);
  }
...
bool TrafficLightTrackComponent::InternalProc(
  ...
  apollo::perf::AppendLatency("perception.traffic_light_tracking.track",
                              trace_id, tr0, tr1);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_tracking.sync_v2x",
                              trace_id, sy0, sy1);
  ...
  apollo::perf::AppendLatency("perception.traffic_light_tracking.transform_output",
                              trace_id, tf0, tf1);
  ...
  const uint64_t w1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.traffic_light_tracking.write",
                              trace_id, w0, w1);
