...
bool CameraLocationRefinementComponent::Proc(
  ...
  apollo::perf::AppendPoint("perception.camera_location_refinement.tick",
                            trace_id, t0);
  ...
  if (!postprocessor_->Process(obstacle_postprocessor_options,
                               out_message.get())) {
    const uint64_t pp1 = apollo::perf::NowNs();
    apollo::perf::AppendLatency("perception.camera_location_refinement.postprocessor.false",
                              trace_id, p0, pp1);
    apollo::perf::AppendLatency("perception.camera_location_refinement.total.false",
                              trace_id, t0, pp1);
    AERROR << "Failed to post process obstacles.";
    return false;
  }
  ...
  apollo::perf::AppendLatency("perception.camera_location_refinement.postprocessor",
                              trace_id, p0, p1);

  const uint64_t w0 = apollo::perf::NowNs();
  writer_->Write(out_message);
  const uint64_t w1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("perception.camera_location_refinement.write",
                              trace_id, w0, w1);
  apollo::perf::AppendLatency("perception.camera_location_refinement.total",
                              trace_id, t0, w1);
  return true;
}
