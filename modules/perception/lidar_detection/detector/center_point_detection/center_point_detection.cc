...
class NvtxRangeGuard {
 public:
  explicit NvtxRangeGuard(const char* msg) {
    nvtxRangePushA(msg);
  }
  ~NvtxRangeGuard() {
    nvtxRangePop();
...
bool CenterPointDetection::InitGpuProfileResources() {
#if GPU_PLATFORM != NVIDIA
  ...
  if (infer_event_start_ == nullptr) {
    if (cudaEventCreate(&infer_event_start_) != cudaSuccess) {
      infer_event_start_ = nullptr;
      AWARN << "cudaEventCreate(infer_event_start_) failed.";
      gpu_event_ready_ = false;
      return false;
    }
  }
  if (infer_event_stop_ == nullptr) {
    if (cudaEventCreate(&infer_event_stop_) != cudaSuccess) {
      AWARN << "cudaEventCreate(infer_event_stop_) failed.";
      if (infer_event_start_ != nullptr) {
...
void CenterPointDetection::ReleaseGpuProfileResources() {
#if GPU_PLATFORM == NVIDIA
  if (infer_event_start_ != nullptr) {
    cudaEventDestroy(infer_event_start_);
    infer_event_start_ = nullptr;
  ...
  if (profile_infer_stream_created_ && profile_infer_stream_ != nullptr) {
    cudaStreamDestroy(profile_infer_stream_);
    profile_infer_stream_ = nullptr;
    profile_infer_stream_created_ = false;
  }
#endif
...
CenterPointDetection::CudaMemSnapshot
CenterPointDetection::ReadCudaMemSnapshot() const {
...
cudaStream_t CenterPointDetection::GetInferenceCudaStream() const {
  if (!inference_) {
    AINFO << "[CPDET_BACKEND_GetInferenceCudaStream] GetInferenceCudaStream inference_=nullptr";
    return static_cast<cudaStream_t>(0);
  }

  cudaStream_t stream = inference_->GetCudaStream();
  AINFO << "[CPDET_BACKEND_GetInferenceCudaStream] GetInferenceCudaStream "
        << "inference_ptr=" << reinterpret_cast<uintptr_t>(inference_.get())
        << " stream=" << reinterpret_cast<uintptr_t>(stream);

  // 注意：这里不直接返回 profile_infer_stream_。
  // 只有 backend 自己的 GetCudaStream() 返回非 0，才认为 stream 可用。
  return stream;
}

bool CenterPointDetection::IsWarmupFrame() const {
  return detect_call_idx_ < gpu_profile_warmup_frames_;
}

bool CenterPointDetection::ShouldInfoLogThisFrame() const {
  if (detect_call_idx_ < gpu_profile_warmup_frames_) {
    return true;
  }
  return (info_log_every_n_frames_ > 0) &&
         (detect_call_idx_ % info_log_every_n_frames_ == 0);
}
//
...
bool CenterPointDetection::Init(const LidarDetectorInitOptions &options) {
  AINFO << "[CPDET_INIT_ENTER] CenterPointDetection::Init entered";
  ...
  AINFO << "[CPDET_FORCE_COMPILE_CHECK] center_point_detection.cc rebuilt";
  #if GPU_PLATFORM == NVIDIA
  AINFO << "[CPDET_BACKEND] "
        << "inference_type=" << typeid(*inference_).name()
        << " stream_before_set="
        << reinterpret_cast<uintptr_t>(inference_->GetCudaStream());
  cudaStream_t backend_stream = inference_->GetCudaStream();

  if (backend_stream == static_cast<cudaStream_t>(0)) {
    cudaError_t stream_ret =
        cudaStreamCreateWithFlags(&profile_infer_stream_,
                                  cudaStreamNonBlocking);
    if (stream_ret == cudaSuccess) {
      profile_infer_stream_created_ = true;
      inference_->SetStream(profile_infer_stream_);

      AINFO << "[CPDET_BACKEND] "
            << "created_profile_stream="
            << reinterpret_cast<uintptr_t>(profile_infer_stream_)
            << " stream_after_set="
            << reinterpret_cast<uintptr_t>(inference_->GetCudaStream());
    } else {
      AWARN << "[CPDET_BACKEND] cudaStreamCreateWithFlags failed: "
            << cudaGetErrorString(stream_ret);
    }
  } else {
    AINFO << "[CPDET_BACKEND] use_backend_stream="
          << reinterpret_cast<uintptr_t>(backend_stream);
  }
  #endif
  ...
  if (cudaSetDevice(model_param_.preprocess().gpu_id()) != cudaSuccess) {
    AERROR << "Failed to set device to gpu "
           ...
           << " while initializing profiler resources.";
    return false;
  }

  InitGpuProfileResources();
  AINFO << "[CPDET_INIT_EXIT] CenterPointDetection::Init finished";
  return true;
}
...
//                                   LidarFrame *frame) {
//   uint64_t trace_id = options.trace_id;
//   if (trace_id == 0 && frame != nullptr && frame->timestamp > 0.0) {
//     trace_id = static_cast<uint64_t>(frame->timestamp * 1e9);
//   }
//   const uint64_t t0 = apollo::perf::NowNs();
//   apollo::perf::AppendPoint("perception.lidar_detection.centerpoint.tick",
//                             trace_id, t0);
//   // check input
//   if (frame == nullptr) {
...
//     "perception.lidar_detection.centerpoint.input_prepare",
//     trace_id, s0, s1);
...
//   apollo::perf::AppendLatency(
//     "perception.lidar_detection.centerpoint.fuse_cloud",
//     trace_id, f0, f1);

//   // shuffle points and cut off
//   const uint64_t sh0 = apollo::perf::NowNs();
...
//   const uint64_t sh1 = apollo::perf::NowNs();
//   apollo::perf::AppendLatency(
//     "perception.lidar_detection.centerpoint.shuffle_indices",
//     trace_id, sh0, sh1);
...
//   apollo::perf::AppendLatency(
//     "perception.lidar_detection.centerpoint.cloud_to_array",
//     trace_id, c0, c1);
...
//   apollo::perf::AppendLatency(
//     "perception.lidar_detection.centerpoint.preprocess",
//     trace_id, p0, p1);
...
//   apollo::perf::AppendLatency(
//     "perception.lidar_detection.centerpoint.inference",
//     trace_id, i0, i1);
//   // paddle inference
...
//   const uint64_t co0 = apollo::perf::NowNs();
...
//   }
//   ADEBUG << sstr.str();
//   const uint64_t co1 = apollo::perf::NowNs();
//   apollo::perf::AppendLatency(
//     "perception.lidar_detection.centerpoint.collect_candidates",
//     trace_id, co0, co1);
    
    
//   const uint64_t po0 = apollo::perf::NowNs();
...
bool CenterPointDetection::Detect(const LidarDetectorOptions &options,
  ...
  // cur_cloud_ptr_初始化
  {
  NvtxRangeGuard range("cpdet.input_prepare");
  // input prepare
  const uint64_t s0 = apollo::perf::NowNs();
  cur_cloud_ptr_ = std::shared_ptr<base::PointFCloud>(
      new base::PointFCloud(*original_cloud_));
  ...
  // down sample the point cloud through filtering voxel grid
  const uint64_t dv0 = apollo::perf::NowNs();
  if (model_param_.preprocess().enable_downsample_pointcloud()) {
    DownSampleOptions down_sample_options;
    ACHECK(down_sample_ != nullptr);
    down_sample_->Process(down_sample_options, cur_cloud_ptr_);
  }
  const uint64_t dv1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency(
    "perception.lidar_detection.centerpoint.downsample_pointcloud",
    trace_id, dv0, dv1);
  ...
  // apollo::perf::AppendLatency(
  //   "perception.lidar_detection.centerpoint.get_objects",
  //   trace_id, g0, g1);
  ...
  // // 1) input shape
  // // ----------------------------
  // const uint64_t ps0 = apollo::perf::NowNs();
  // std::vector<int> points_shape{
  //     num_points, model_param_.preprocess().num_point_feature()};
  // const uint64_t ps1 = apollo::perf::NowNs();
  // apollo::perf::AppendLatency(
  //     "perception.lidar_detection.centerpoint.input_shape",
  //     trace_id, ps0, ps1);
  ...
  // apollo::perf::AppendLatency(
  //     "perception.lidar_detection.centerpoint.input_blob_reshape",
  //     trace_id, r0, r1);

  // // ----------------------------
  // // 3) get cpu buffer of blob
  // // ----------------------------
  // const uint64_t mc0 = apollo::perf::NowNs();
  // float *data_ptr = input_data_blob->mutable_cpu_data();
  // const uint64_t mc1 = apollo::perf::NowNs();
  // apollo::perf::AppendLatency(
  //     "perception.lidar_detection.centerpoint.input_blob_mutable_cpu",
  //     trace_id, mc0, mc1);

  // // ----------------------------
  // // 4) memcpy to blob cpu buffer
  // // ----------------------------
  // const uint64_t cp0 = apollo::perf::NowNs();
  // memcpy(data_ptr, points_data.data(), points_data.size() * sizeof(float));
  // const uint64_t cp1 = apollo::perf::NowNs();
  // apollo::perf::AppendLatency(
  //     "perception.lidar_detection.centerpoint.input_blob_memcpy_cpu",
  //     trace_id, cp0, cp1);

  // // 可选：把输入规模打到日志里，便于后面分析 num_points 与推理时长的关系
  // AINFO << "[CenterPointDetection Input] trace_id=" << trace_id
  //       << " num_points=" << num_points
  //       << " num_point_feature="
  //       << model_param_.preprocess().num_point_feature()
  //       << " points_data_size=" << points_data.size();

  // // ----------------------------
  // // 5) inference wall time + gpu event time
  // // ----------------------------
  ...
  // if (cudaEventCreate(&infer_start) != cudaSuccess) {
  //   event_ok = false;
  //   AWARN << "cudaEventCreate(infer_start) failed.";
  // }
  // if (event_ok && cudaEventCreate(&infer_stop) != cudaSuccess) {
  //   event_ok = false;
  //   AWARN << "cudaEventCreate(infer_stop) failed.";
  // }

  // // 最小改动版本：先用默认 stream 0
  // if (event_ok) {
  //   if (cudaEventRecord(infer_start, 0) != cudaSuccess) {
  //     event_ok = false;
  //     AWARN << "cudaEventRecord(infer_start) failed.";
  //   }
  // }
  ...
  // if (event_ok) {
  //   if (cudaEventRecord(infer_stop, 0) != cudaSuccess) {
  //     event_ok = false;
  //     AWARN << "cudaEventRecord(infer_stop) failed.";
  //   }
  // }

  // if (event_ok) {
  //   if (cudaEventSynchronize(infer_stop) != cudaSuccess) {
  //     event_ok = false;
  //     AWARN << "cudaEventSynchronize(infer_stop) failed.";
  //   }
  // }

  // if (event_ok) {
  //   if (cudaEventElapsedTime(&infer_gpu_ms, infer_start, infer_stop) !=
  //       cudaSuccess) {
  //     event_ok = false;
  //     AWARN << "cudaEventElapsedTime failed.";
  //   }
  // }

  // if (event_ok) {
  //   // 说明：
  //   // 这里为了兼容你现在的 latency CSV 管线，仍然写成一个 latency 事件。
  //   // 但它的 begin/end 是“用 wall 起点 + GPU时长”拼出来的，
  //   // 表示“持续时间序列”，不是严格真实的时间线起止。
  //   const uint64_t ig0 = i0;
  //   const uint64_t ig1 =
  //       i0 + static_cast<uint64_t>(infer_gpu_ms * 1000000.0f);
  //   apollo::perf::AppendLatency(
  //       "perception.lidar_detection.centerpoint.inference_gpu_event",
  //       trace_id, ig0, ig1);

  // }
  // const uint64_t i1 = apollo::perf::NowNs();
  // apollo::perf::AppendLatency(
  //     "perception.lidar_detection.centerpoint.inference_wall",
  //     trace_id, i0, i1);

  // if (infer_start != nullptr) {
  //   cudaEventDestroy(infer_start);
  // }
  // if (infer_stop != nullptr) {
  ...
  // // 6) filter score
  // // ----------------------------
  // const uint64_t fs0 = apollo::perf::NowNs();
  // FilterDiffScore(output_bbox_blob, output_label_blob, output_score_blob,
  //                 &out_detections, &out_labels, &out_scores);
  // const uint64_t fs1 = apollo::perf::NowNs();
  // apollo::perf::AppendLatency(
  //     "perception.lidar_detection.centerpoint.filter_diff_score",
  //     trace_id, fs0, fs1);
  ...
  const bool warmup = IsWarmupFrame();
  ++detect_call_idx_;

  double inference_wall_ms = -1.0;
  ...
  // 5) memory snapshot before infer
  CudaMemSnapshot mem_before = ReadCudaMemSnapshot();

  // 6) get real inference stream
  // const bool infer_stream_available =
  //     (infer_stream != static_cast<cudaStream_t>(0));

  // bool gpu_event_ok = gpu_event_ready_ && infer_stream_available;
  cudaStream_t infer_stream = GetInferenceCudaStream();

  const bool infer_stream_is_default =
      (infer_stream == static_cast<cudaStream_t>(0));

  // stream 0 是合法 CUDA default stream，不应判定为不可用
  bool gpu_event_ok = gpu_event_ready_;

  float infer_gpu_ms = -1.0f;

  // 7) NVTX infer range
  {
    NvtxRangeGuard infer_range("cpdet.infer");

  #if GPU_PLATFORM == NVIDIA
      if (enable_cuda_profiler_api_ && !warmup) {
        cudaProfilerStart();
      }
  #endif
    ...
    if (gpu_event_ok) {
      if (cudaEventRecord(infer_event_start_, infer_stream) != cudaSuccess) {
        gpu_event_ok = false;
        AWARN << "cudaEventRecord(infer_event_start_) failed.";
      }
    }
    ...
    if (gpu_event_ok) {
      if (cudaEventRecord(infer_event_stop_, infer_stream) != cudaSuccess) {
        gpu_event_ok = false;
        AWARN << "cudaEventRecord(infer_event_stop_) failed.";
      }
    }
    ...
    if (gpu_event_ok) {
      if (cudaEventSynchronize(infer_event_stop_) != cudaSuccess) {
        gpu_event_ok = false;
        AWARN << "cudaEventSynchronize(infer_event_stop_) failed.";
      }
    }
    ...
    inference_wall_time_ = inference_wall_ms;
    inference_time_ = inference_wall_ms;
  }

  // 8) memory snapshot after infer
  CudaMemSnapshot mem_after = ReadCudaMemSnapshot();

  double mem_before_used_mib = -1.0;
  double mem_after_used_mib = -1.0;
  double mem_delta_used_mib = -1.0;

  if (mem_before.ok) {
    mem_before_used_mib =
        BytesToMiB(mem_before.total_bytes - mem_before.free_bytes);
  }

  if (mem_after.ok) {
    mem_after_used_mib =
        BytesToMiB(mem_after.total_bytes - mem_after.free_bytes);
  }

  if (mem_before.ok && mem_after.ok) {
    mem_delta_used_mib = mem_after_used_mib - mem_before_used_mib;
  }
  #if GPU_PLATFORM == NVIDIA
  AINFO << "[CPDET_PLATFORM_CHECK] GPU_PLATFORM=NVIDIA";
  #elif GPU_PLATFORM == AMD
    AINFO << "[CPDET_PLATFORM_CHECK] GPU_PLATFORM=AMD";
  #else
    AINFO << "[CPDET_PLATFORM_CHECK] GPU_PLATFORM=UNKNOWN_OR_CPU";
  #endif
  if (ShouldInfoLogThisFrame()) {
    AINFO << std::fixed << std::setprecision(3)
      << "[CPDET_INFER_PROFILE] "
      << "trace_id=" << trace_id
      << " warmup=" << (warmup ? 1 : 0)
      << " detect_idx=" << detect_call_idx_
      << " num_points=" << num_points
      << " shape0=" << points_shape[0]
      << " shape1=" << points_shape[1]
      << " infer_stream_addr="
      << reinterpret_cast<uintptr_t>(infer_stream)
      << " infer_stream_is_default=" << (infer_stream_is_default ? 1 : 0)
      << " inference_wall_ms=" << inference_wall_ms
      << " inference_gpu_event_ok=" << (gpu_event_ok ? 1 : 0)
      << " inference_gpu_event_ms=" << infer_gpu_ms
  ...
  // 下面 FilterDiffScore / GetObjects / postprocess 全部保留
  {
    NvtxRangeGuard post_range("cpdet.postprocess");
    std::vector<float> out_detections;
    std::vector<int64_t> out_labels;
    std::vector<float> out_scores;
    ...
    postprocess_time_ = timer.toc(true);
    const uint64_t po1 = apollo::perf::NowNs();
    apollo::perf::AppendLatency(
      "perception.lidar_detection.centerpoint.postprocess",
      trace_id, po0, po1);
    ...
    apollo::perf::AppendLatency(
      "perception.lidar_detection.centerpoint.total",
      trace_id, t0, t1);
    AINFO << "[CPDET_FRAME] "
      << "trace_id=" << trace_id
      << " num_points=" << num_points
      ...
      << " shape1=" << points_shape[1]
      << " inference_wall_ms=" << inference_wall_ms
      << " inference_gpu_ms=" << infer_gpu_ms
      << " detect_total_ms=" << (t1 - t0) / 1000000.0;
  }
  return true;
}
