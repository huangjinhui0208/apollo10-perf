...
Status ControlComponent::ProduceControlCommand(
  ...
  uint64_t trace_id = 0;
  if (local_view_.trajectory().has_header()) {
    trace_id =
        apollo::perf::TraceIdFromHeader(local_view_.trajectory().header());
  }

  const uint64_t s0 = apollo::perf::NowNs();
  Status status = CheckInput(&local_view_);
  const uint64_t s1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("control.check_input", trace_id, s0, s1);

  if (status.ok()) {
    const uint64_t s2 = apollo::perf::NowNs();
    Status status_ts = CheckTimestamp(local_view_);
    const uint64_t s3 = apollo::perf::NowNs();
    apollo::perf::AppendLatency("control.check_timestamp", trace_id, s2, s3);
    (void)status_ts;
  }
  ...
  if (!local_view_.trajectory().trajectory_point().empty()) {
    const uint64_t s4 = apollo::perf::NowNs();
    ...
    const uint64_t s5 = apollo::perf::NowNs();
    apollo::perf::AppendLatency("control.solve", trace_id, s4, s5);
...
bool ControlComponent::Proc() {
  ...
  const uint64_t c0 = start_time.ToNanosecond();

  // proc_enter - 调度时刻，调度抖动
  apollo::perf::AppendPoint("control.proc_enter", 0, c0);
  ...
  if (planning_status_msg) {
  injector_->set_planning_command_status(*planning_status_msg);
}
  ...
  // observe_done - 数据拉取耗时，各路数据 age
  const uint64_t obs0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("control.observe_done", 0, obs0);

  // localview_ready - 数据就绪
  const uint64_t lv0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("control.localview_ready", 0, lv0);
  ...
  apollo::perf::AppendPoint("control.tick", trace_id, c0);
  ...
  // automode_check - 驾驶模式状态
  const uint64_t auto0 = apollo::perf::NowNs();
  CheckAutoMode(&local_view_.chassis());
  const uint64_t auto1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("control.automode_check", trace_id, auto0, auto1);

  ControlCommand control_command;

  // checkinput_enter - CheckInput 起点
  const uint64_t ci0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("control.checkinput_enter", trace_id, ci0);
  ...
  // checkinput_exit 和 checktimestamp_result 已在 ProduceControlCommand 中记录
  ...
  if (estop_) {
    // estop_trigger - estop 触发时刻和原因
    const uint64_t estop0 = apollo::perf::NowNs();
    apollo::perf::AppendPoint("control.estop_trigger", trace_id, estop0);
  ...
  apollo::perf::AppendLatency("control.write_done", trace_id, w0, w1);
  ...
  const uint64_t c1 = end_process_control_time.ToNanosecond();
  apollo::perf::AppendLatency("control.total", trace_id, c0, c1);
  apollo::perf::AppendPoint("control.output_pub", trace_id, c1);
