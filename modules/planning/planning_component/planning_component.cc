...
bool PlanningComponent::Proc(
  ...
  if (prediction_obstacles && prediction_obstacles->has_header()) {
    trace_id = apollo::perf::TraceIdFromHeader(prediction_obstacles->header());
  }
  AINFO << "[TRACE_DEBUG_PLAN_IN] "
      << "trace_id=" << apollo::perf::TraceIdFromHeader(prediction_obstacles->header())
      << " header=" << prediction_obstacles->header().DebugString()
      << " chassis_header=" << chassis->header().ShortDebugString()
      << " localization_header=" << localization_estimate->header().ShortDebugString();

  // proc_enter - Proc 总起点
  const uint64_t p0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("planning.proc_enter", trace_id, p0);
  ...
  // localview_ready - 数据就绪时刻
  const uint64_t lv0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("planning.localview_ready", trace_id, lv0);

  // checkinput_result - CheckInput() 返回后
  const uint64_t ci0 = apollo::perf::NowNs();
  bool input_ok = CheckInput();
  const uint64_t ci1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("planning.check_input", trace_id, ci0, ci1);
  apollo::perf::AppendPoint("planning.checkinput_result", trace_id, ci1);
  ...
  // runonce_enter - 规划核心起点
  const uint64_t r0 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("planning.runonce_enter", trace_id, r0);

  planning_base_->RunOnce(local_view_, &adc_trajectory_pb);

  // runonce_exit - 规划核心耗时终点
  const uint64_t r1 = apollo::perf::NowNs();
  apollo::perf::AppendPoint("planning.runonce_exit", trace_id, r1);
  ...
  apollo::perf::AppendLatency("planning.write_traj", trace_id, w0, w1);
  apollo::perf::AppendPoint("planning.output_pub", trace_id, w1);
  ...
  // cmd_status_write - 命令状态发布耗时
  const uint64_t cs0 = apollo::perf::NowNs();
  command_status_writer_->Write(command_status);
  const uint64_t cs1 = apollo::perf::NowNs();
  apollo::perf::AppendLatency("planning.cmd_status_write", trace_id, cs0, cs1);
  ...
  // total - Proc 完整终点
  const uint64_t total_end = apollo::perf::NowNs();
  apollo::perf::AppendLatency("planning.total", trace_id, p0, total_end);
